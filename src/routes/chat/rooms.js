...
// const adminsList = require('../../helpers/adminUsers');
...
module.exports = async function process(req, res) {
  ...
  // if (adminsList.indexOf(req.user.steamId) > -1) {
  //   const generalRoom = await getGeneralRoom(req.user);
  //   rooms.unshift(generalRoom.room);
  // }

  const newMessages = rooms.reduce((sum, r) => (sum || 0) + r.newMessages, 0);
  res.json({ status: 'success', rooms, newMessages });
};
